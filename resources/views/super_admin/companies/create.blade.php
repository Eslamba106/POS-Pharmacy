        ...
        <div class="col-lg-6 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{ __('companies.company_info') }}</h5>
                </div>
                ...
                <form class="form-horizontal" action="{{ route('admin.companies.store') }}" method="POST"
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            <label class="col-sm-3 col-from-label" for="mobile">{{ __('login.company_id') }}</label>
                            <div class="col-sm-9">
                                <input type="text" placeholder="{{ __('login.company_id') }}"   name="company_id"
                                    class="form-control"  >
                            </div>
                            @error('company_id')
                                <span class="text-red">
                        ...
                        <div class="form-group row">
                            <label class="col-sm-3 col-from-label" for="branches_count">{{ __('companies.branches_count') }}</label>
                            <div class="col-sm-9">
                                <input type="text" placeholder="{{ __('companies.branches_count') }}"   name="branches_count"
                                    class="form-control"  >
                            </div>
                            @error('branches_count')
                                <span class="text-red">
                                    {{ $errors->first('branches_count') }}
                                </span>
                            @enderror
                        </div> 
                        ...
                        <div class="form-group mb-0 @if (session()->get('locale') == 'en')
                            text-right
                        @else
                            text-left
                        @endif ">
                            <button type="submit" class="btn btn-primary rounded-0 w-150px">{{ __('general.save') }}</button>
                        </div>
                    </div>
                </form>
